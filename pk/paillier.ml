...
let pub ~g ~n  =
    ...
    let* () = guard Z.( n > zero && numbits n >= minimum_bits) ( `Msg "invalid n" ) in
    Ok {g; n; n2}
...
let priv ~p ~q =
    ...
    let* () = guard Z.((gcd (mul p q) (mul (pred p) (pred q)))=one) (`Msg "gcd(p.q, (p-1).(q-1)) = 1") in
    let n = Z.(mul p q) in
    let g = Z.(succ n) in
    let n2 = Z.(mul n n) in
...
let encrypt ~(pub_key : pub) ~msg ?r () =
    if msg < Z.zero then
        invalid_arg "Paillier.encrypt: msg must be non-negative"
    else if msg >= pub_key.n then
        invalid_arg "Paillier.encrypt: msg must be less than n";
    let check_r r = Z.(r > zero && r < pub_key.n && (gcd r pub_key.n) = one) in
    let rec get_r () =
        let r = random_bigint_range ~lower:Z.one ~upper:Z.(pred pub_key.n) in
        if check_r r then r else get_r () in
    let r = match r with
    | None -> get_r ()
    | Some r -> let _ = guard (check_r r) (`Msg "invalid r") in r in
